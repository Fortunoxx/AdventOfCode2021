...
def calcLastValidTargetX(coordinates):
    xmin = coordinates[0][0]
    xmax = coordinates[1][0]
    i = 0 
    x = 0
    ...
    # x is the spot where we will hit the target 
    # i is the maximum x-velocity when this can happen 
    return (lastValidX, i-1) 
...
def process(fileInfos):
    for fileInfo in fileInfos:
        ...
        result = {"file": fileInfo['key'], "maxHeight": maxHeight }
...
def process2(fileInfos):
    for fileInfo in fileInfos:
        ...
        result = {"file": fileInfo['key'], "velocities": len(velocities) }
